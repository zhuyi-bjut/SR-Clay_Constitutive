...
# Import data sets
train = pd.read_excel(r'Shear strain10.xlsx')

features = [x for x in train.columns
            if x not in ['ev', 'ev-1', 's', 'a3', 'G']]
...
function my_custom_objective(tree, dataset::Dataset{T}, options) where {T<:Real}
    ...
    # Boundary condition : For volume strain only : dp component > dp / K when n = 0
    # The dp component is divided
    # Evaluate f1:
    ...
    !flag && return T(10000)    
    # To determine dp component > dp / K
    if f1_value[1]<0.000311 || f1_value[2]<0.000145
        boundary_loss = 666
    else
        boundary_loss = 0
    end
...
model = PySRRegressor(
    ...
    unary_operators=["square"],  # "acs(x)=1/square(x)"],# Unary operator ^ Custom operator( julia)"inv(x) = 1/x",
    # extra_sympy_mappings={"erbei": lambda x: x*2},
    # "special": lambda x,y: cos(x) * (x + y)},
    # loss="loss(prediction, target) = abs(prediction - target)",
    loss_function=objective,
    # ^ self-defined loss function(julia syntax)
    maxsize=50,
    # ^ Allow greater complexity.
    maxdepth=50,
    # ^ Avoid deep nesting.
    complexity_of_constants=100,  # Constant complexity
    precision=64,
    ...
    # annealing=True,
    denoise=True,
    # weight_delete_node=3, # delete nodes
    # weight_randomize=1,  # Random weight
    # weight_add_node=10,  # vertex addition
